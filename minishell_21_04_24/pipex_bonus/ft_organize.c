...
/*   Updated: 2024/04/22 06:44:14 by rrimonte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_order_outfiles2(t_data *g_data, t_data_list *temp, t_data_list *aux)
{
	temp = g_data->cmd_lst;
			...
			&& g_data->cmd_lst->s_tg == g_data->cmd_lst->next->s_tg)
		{
			if ((g_data->cmd_lst->next->outfile_tag == 0)
				|| (g_data->cmd_lst->next->outfile_tag == 1
					&& g_data->cmd_lst->next->infile_tag == 1))
				ft_order_outfiles2(g_data, temp, aux);
		}
	}
}

void	ft_order_infiles2(t_data *g_data, t_data_list *temp, t_data_list *aux)
{
	temp = g_data->cmd_lst;
...
void	ft_order_infiles(t_data *g_data)
{
		...
		&& g_data->cmd_lst->s_tg == g_data->cmd_lst->previous->s_tg)
	{
		if ((g_data->cmd_lst->previous->infile_tag == 0)
			|| (g_data->cmd_lst->previous->outfile_tag == 1
				&& g_data->cmd_lst->previous->infile_tag == 1))
			ft_order_infiles2(g_data, temp, aux);
	}
}
